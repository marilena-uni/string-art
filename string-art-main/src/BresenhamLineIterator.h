...
public:
    ...
    { //devo capire se voglio andare vert o orizz, e mi sposto la maggiornaza
        id = std::abs(delta[0]) >= std::abs(delta[1]) ? 0 : 1;

        //decide se andare piu a destra o piu a sx/sopra  o sotto sia su x che su y
    ...
    BresenhamLineIterator operator++()
    {
        ...
        error += 2 * delta[1 - id]; //aggiorno erroe
        return *this;
    }
